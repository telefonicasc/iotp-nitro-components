      ...
      this.after('initialize', function() {
        ...
        if (this.attr.clipRange) {
          ...
          clipRange = d3.select(this.node.parentNode).select('defs')
            .append('clipPath').attr('id', clipRangeId).append('rect');
          this.context.attr('clip-path', 'url(#' + clipRangeId + ')');
        } else {
          if (this.attr.clipId) {
            this.context.attr('clip-path', 'url(#' + this.attr.clipId + ')');
          }
        }
        ...
        this.on('valueChange', function(e, options) {
          ...
          if (options.range && value) {
            this.value = $.map(value, function(item) {
              if (item[xkey] >= options.range[0] && item[xkey] <= options.range[1]) {
                return item;
              }
            });

            if (clipRange && model[this.attr.clipRange]) {
              rangeStartx = this.scalex(model[this.attr.clipRange][0]);
              rangeEndx = this.scalex(model[this.attr.clipRange][1]);
              clipRange 
                .attr('x', rangeStartx)
